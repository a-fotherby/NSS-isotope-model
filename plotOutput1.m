classdef plotOutput
    methods(Static)
        ...
        function [] = depthPlot(d34, depth, lables)
            f = figure
            set(f,'Units','normal','Position',[0.1 0.2 0.8 0.7])
            set(gca,'YDir','reverse','YLim',[0 1000],'XLim',[0 30],'XAxisLocation','Top','FontName','Calibri','FontSize',[10]);
            xlim([20 80])
            ylim([0 1000])
            hold on
            ...
            while l < plotNum
                plot(d34(:,l),depth(:,1), 'color', cc(l, :), 'DisplayName', sprintf('# 2^{%d} %0.5f %%',l, lables(1,l)))
