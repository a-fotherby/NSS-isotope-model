...
while RMSSO4 > steadyStateLimit
    ...
    % This way around because reduction value is subtracted in the iterate method.
    ORR16_t = SRR_16 - b_16;
    ORR18_t = SRR_18 - b_18;
    ...
    O_sum = SO4_16 + SO4_18;
    check = SO4_sum - O_sum;
    % Picked value, seems to work best.
    safety = 0.1; 
    
    SRR_32 = safe.rate(SRR_32, SO4_sum, safety);
    SRR_33 = safe.rate(SRR_33, SO4_sum, safety);
    SRR_34 = safe.rate(SRR_34, SO4_sum, safety);
    
    SRR_t = safe.rate(SRR_t, SO4_sum, safety);
    
    ORR16_t = safe.rate(ORR16_t, O_sum, safety);
    ORR18_t = safe.rate(ORR18_t, O_sum, safety);
    ...
    %% Safety code for concentrations after iteration.
    % Recalculate total concentrations for each species after iteration.
    SO4_sum = SO4_32 + SO4_33 + SO4_34;
    O_sum = SO4_16 + SO4_18;
  
    SO4_sum = safe.concentration(SO4_sum, SO4_sum);
    SO4_32 = safe.concentration(SO4_32, SO4_sum);
    SO4_33 = safe.concentration(SO4_33, SO4_sum);
    SO4_34 = safe.concentration(SO4_34, SO4_sum);
    
    O_sum = safe.concentration(O_sum, O_sum);
    SO4_16 = safe.concentration(SO4_16, O_sum);
    SO4_18 = safe.concentration(SO4_18, O_sum);
    
    ORR16_t = safe.concentration(ORR16_t, O_sum);
    ORR18_t = safe.concentration(ORR18_t, O_sum);
